function MCHP_ARM7_stf_make_rtw_hook(hookMethod,modelName,rtwroot,templateMakefile,buildOpts,buildArgs,buildInfo)
...
switch hookMethod
    case 'error'
        ...
        try
			...
			%MCHP_CreateMPLAB(modelName,'ARM',ProjectRelativePath_Root,buildInfo);           
    ...
    case 'entry'
        ...
        if 0            
            if NbrPins > 8
                error('***** Pin used exceed the max number of pin allowed by your license *****');
            end
        end
    ...
    case 'before_tlc'
        ...
        % RTWGenSettings not available with XCP on R2018b
        % folderStruct = RTW.getBuildDir(model)
        if 1
            RTWGenSettings = get_param(modelName,'RTWGenSettings');
            if isfield(RTWGenSettings,'RelativeBuildDir')
                RTWGenSettings.RelativeBuildDir;    %% with R2018b, with XCP, bug here
                rtwFile = [RTWGenSettings.RelativeBuildDir filesep modelName '.rtw'];
            else
                pX = mchp.prefs.getFolderNameExtension;
                rtwFile = [pwd filesep modelName pX filesep modelName '.rtw'];
            end
    ...
    case 'before_make'
        ...
        % buildInfo.getFullFileList
        % buildInfo.getSourceFiles('','')
        % buildInfo.getIncludeFiles(1,1)
        % buildInfo.getIncludeFiles('','')
        % buildInfo.getIncludePaths('','')*
        % buildInfo.getSourcePaths(1)
        % buildInfo.getLibraryPaths
        % buildInfo.getMasterAnchorDir
        % buildInfo.getLocalBuildDir
		if 1
			try
                CurrentFolder = pwd;
		        if i_isModelRef(modelName) == 0           
		            MCHP_CreateMPLAB(modelName,'ARM',buildInfo);
				end
 	    	catch
    ...
    case 'exit'
        ...
        Flag = 0;
        try
            if isfield(buildOpts,'XilInfo') && (buildOpts.XilInfo.IsStandaloneBuild == 1) ||...  %% For old matlab release, do not test this.
                    ~isfield(buildOpts,'XilInfo')
                Flag = 1;
            end
        catch
            Flag = 1;
