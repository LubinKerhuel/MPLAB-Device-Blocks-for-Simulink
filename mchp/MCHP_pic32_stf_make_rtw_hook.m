function MCHP_pic32_make_rtw_hook(hookMethod,modelName,rtwroot,templateMakefile,buildOpts,buildArgs,buildInfo)
...
switch hookMethod
    case 'error'
        ...
        try
			...
			%MCHP_CreateMPLAB(modelName,'PIC32',ProjectRelativePath_Root,buildInfo);
...
%        end
    ...
    case 'before_tlc'
        ...
        if i_isModelRef(modelName) && i_isPilSim(buildOpts)
            rtwFile = ['slprj' filesep 'MCHP_PIC32_stf' filesep modelName filesep modelName '.rtw'];
        else
		...
		if 1
			try
		        ...
		        if i_isModelRef(modelName) == 0
		            MCHP_CreateMPLAB(modelName,'PIC32',buildInfo);
				end
	    	catch
