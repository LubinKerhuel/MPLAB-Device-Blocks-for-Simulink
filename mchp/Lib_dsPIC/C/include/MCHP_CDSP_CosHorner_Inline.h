#ifndef CDSP_COS_HORNER_H
#define CDSP_COS_HORNER_H
...
static volatile _Fract CoeffValue0 =  0.0191955566r;
static volatile _Fract CoeffValue1 = -0.25271606r; 
static volatile _Fract CoeffValue2 =  0.6167602539r; 
static volatile _Fract CoeffValue3 = -1r;
static volatile _Fract ConstValue  =  0.5r;      

static inline unsigned int MCHP_CDSP_CosHorner_Inline(signed int u1, unsigned int u2, unsigned int u3) 
{
    ...
    //--------------------- Scale & modulo ----------------
    x.s = input1.s * input2.u +  (int) ((unsigned long) input3.u * input1.s >> 16);    /* x.s is a normalized angle ([-1 1]) ; any wrap act as a modulo which is good */
    normalizedAngleInput.s = x.s;                       /* Save value for post processing solution reconstruction */
    ...
    y.f = (short _Accum)x.f*y.f + CoeffValue1;
    y.f = (short _Accum)x.f*y.f + CoeffValue2;   /* Coef 2 is bigger than 1. We divided it by 2, and perform the operation twice */
	temp.f = (short _Accum)x.f*CoeffValue2 + CoeffValue3;
    y.f = (short _Accum)x.f*y.f + temp.f;        /* see coef 2 repeated here */
    
    //--------------------- Reconstruct --------------------------
	y.f = (short _Accum)y.f * ConstValue;     //y.f = (short _Accum)y.f * 0.5r; //to comply with Simulink output (fixdt(1,16,14)); LSB is 2^-14
	
    if ( (normalizedAngleInput.u +  16384u) < 0x8000){ /* Benefit from wrap effect to detect quadrant requiring to negate the output*/
        y.f = -y.f;                 /* negate output if quadrant required it */
    }  
    //-------------------- Return result ----------------------
